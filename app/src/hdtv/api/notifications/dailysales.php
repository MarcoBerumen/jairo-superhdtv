...
dispatch('/public/notifications/sales/send',function(){
...
//        print_r($store);
        ...
        if(count($emails)>0){
            ...
            $path = "https://{$_SERVER['HTTP_HOST']}/public/notifications/sales/?store={$store['store_id']}/";
            $url = urlencode($path);
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = 'Daily Sales Report '.$date .' '.$store['name'];
        $mail->Body    = 'Daily Sales Report';
        $mail->send();
